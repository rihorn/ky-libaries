...
function(declare, _WidgetBase, _TemplatedMixin, _WidgetsInTemplateMixin, template, lang, html,
  array, on, query, SimpleTable, TextBox, Select, TooltipDialog, Menu, MenuItem, dojoPopup) {
  return declare([_WidgetBase,_TemplatedMixin,_WidgetsInTemplateMixin], {
    ...
    setConfig:function(config){
      ...
      if(this.config.fields && this.config.fields.length > 0){
        this._setFields(this.config.fields, this.config.order);
      }
      
      this.orderSelect.attr("value", config.order);
    ...
    getConfig:function(){
      ...
      config.order = this.orderSelect.get('value');
    ...
    _setFields:function(fields, order){
      var select = this.orderSelect;
      var option;
      this._resetMenu();
      ...
      if (this.fields.length > 0) {
        ...
        array.forEach(this.fields, lang.hitch(this, function(fieldInfo) {
    		option = { value: fieldInfo.name, label: fieldInfo.name, selected: false };
      		select.addOption([option]);
          	this._addMenuItem(fieldInfo);
          	this._addRow(fieldInfo);
        }));
        select.attr("value", order);
      } else {
        this._addEmptyMenuItem();
      }
    },
